...
public class SuccessMessageInterceptor implements Serializable {
	...
	public Object interceptarMetodo(InvocationContext context) throws Exception {
		try {
			LOG.info(String.format("ANTES INVOCAR %s.%s", context.getMethod().getDeclaringClass().getName(), context.getMethod().getName()));
			...
			LOG.info(String.format("APÓS INVOCAR %s.%s", context.getMethod().getDeclaringClass().getName(), context.getMethod().getName()));
		}
	}
